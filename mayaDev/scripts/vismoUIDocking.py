...
class Example_connectAttr(MayaQWidgetDockableMixin, QtWidgets.QScrollArea):
    ...
    # def testPanel(self):
    #     attrs = ['amplitude', 'size', 'time1', 'time2', 'time3', 'pointSpaceLocal']
    #     # Create container for attr widgets
    #     self.setWindowTitle('ConnectAttrs: %s'%self.nodeName)
    #     self.attrUI = QtWidgets.QWidget(parent=self)
    #     layout = QtWidgets.QHBoxLayout()          
    #     hello = QtWidgets.QLabel("Hello, World") 
    #     layout.addWidget(hello,0, QtCore.Qt.AlignLeft) 
    ...
    def attachToNode(self, nodeName):
        ...
        for attr in attrs:
            ...
            if attr == 'pointSpaceLocal':
                pointSpaceLocalCheckbox = QtWidgets.QCheckBox(parent=self.attrUI)
                pointSpaceLocalCheckbox.setChecked(v)   
                pointSpaceLocalCheckbox.stateChanged.connect( lambda: self.checkBoxChange(pointSpaceLocalCheckbox, attr) )        
                # Add to layout
                layout.addRow(attr, pointSpaceLocalCheckbox)
                # Track the widget associated with a particular attr
                self.attrWidgets[attr] = pointSpaceLocalCheckbox
                
            else:    
                attrValueWidget = QtWidgets.QLineEdit(parent=self.attrUI)
    ...
    def checkBoxChange(self, widget, attrName):
        # setAttr "pSphere1TrblzGrp.pointSpaceLocal" 0;
        print widget.isChecked()
        if widget.isChecked() == True:
            cmds.setAttr('%s.%s'%(self.nodeName, attrName), 1)
        else:
            cmds.setAttr('%s.%s'%(self.nodeName, attrName), 0)
